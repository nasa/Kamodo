...
def get_unit_quantity(name, base, scale_factor, abbrev=None, unit_system='SI'):
    ...
    u = sympy_units.Quantity(name, abbrev=abbrev)
...
# Not used by Kamodo-core. remove?
def compile_fortran(source, module_name, extra_args='', folder = './'):
    """Compiles a Fortran module from input source code"""
...
def arg_to_latex(arg):
    """Convert this argument into a latex string"""
...
# # This code is not needed by Kamodo-core 
...
def sort_symbols(symbols):
    """returns the sorted set of symbols"""
    symbols_ = list(symbols)
    symbols_.sort(key=str)
    return tuple(symbols_)


def valid_args(func, kwargs):
    '''Extract arguments from kwargs that appear in func'''
    valid = OrderedDict()
    if isinstance(func, np.vectorize):
        for arg in getfullargspec(func.pyfunc).args:
            if arg in kwargs:
                valid[arg] = kwargs[arg]
    else:
...
def get_defaults(func):
    """exctracts the default parameters from func"""
    sig = inspect.signature(func)
    defaults = {}
    for arg, value in sig.parameters.items():
        if value.default is not inspect._empty:
            defaults[arg] = value.default

    return defaults

# this function isn't used by kamodo core
...
def cast_0_dim(array, other):
    """converts a zero-dimensional array to the array shape of other"""
    if array.ndim == 0:
        return array*np.ones(other.shape)
    return array
...
def concat_solution(gen, variable):
    """concatonate a series of solutions"""
    result = []
...
# not sure where existing_plot_types is used in kamodo core. Could just be in notebook examples
...
# should use python forge here to avoid this messiness
...
# @pointlike is an experimental decorator. I don't think it's used in kamodo core.

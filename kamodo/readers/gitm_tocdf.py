...
def _read(varlist, attrs, newfile=True):
    ...
    for val in var:  #loops through all variable data stored in file
        ...
        if gvar:
            ...
            elif v == 'Altitude': coords['height'] = np.unique(np.array(temp))/1000. #km
...
def gitm_toCDF(filename, coords, variables, var_dict, attrs):
    ...
    for variable_name in variables.keys():  
        ...
        if new_name not in var_3D:
            new_var = data_out.createVariable(new_name, np.float32, ('time','height','lat','lon'))
        else:
            new_var = data_out.createVariable(new_name, np.float32, ('time','lat','lon'))
...
def GITMbin_toCDF(file_prefix):
    ...
    print(f'Files of pattern {file_prefix+"*.bin"} converted to {cdf_filename} '+\
          f'in {ti.perf_counter()-ftic:.5f}s')
    return True
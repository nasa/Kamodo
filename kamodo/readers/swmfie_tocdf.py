...
def _read_SWMFIE(file_prefix, verbose=False): 
    ...
    files = glob.glob(file_prefix+'*.tec')   #take one day of data
    ...
    #t_header = ti.perf_counter()
    file_header = read_SWMFIE_header(files[1])  #first file has a different header format
    
    #if verbose: print(f'Took {ti.perf_counter()-t_header:.6f}s to parse header.')
    ...
    coords = {'time':time, 'height':height, 'lat':lat, 'lon':lon}
    variables['theta_Btilt'], variables['psi_Btilt'] = theta_Btilt, psi_Btilt
    var_units['theta_Btilt'], var_units['psi_Btilt'] = 'deg','deg'
    if verbose: print(f'Took {ti.perf_counter()-t0:.6f}s to read and '+\
                      f'assemble data fom {len(files)} files.')
...
def _toCDF(filename, files, coords, variables, var_units, filedate):
    ...
    for dim in coords.keys():  #check that the datatype works correctly when opening the file******
        ...
        new_var = data_out.createVariable(dim, np.float32, dim)  #create variable
        new_var[:] = coords[dim]  #store data for dimension in variable
        if dim=='height': units = 'km'
        elif dim=='time': units = 'hrs'
        else: units='deg'
        new_var.units = units
    ...
    for variable_name in variables.keys(): 
        if variable_name not in var_1D:
            new_var = data_out.createVariable(variable_name, np.float32, ('time','lat','lon'))
        else:
...
def convertSWMFIE_toCDF(file_prefix):
    ...
    print('NetCDF version of data not found. Converting files with given prefix to netCDF.')
    ftic = ti.perf_counter()
    ...
    print(f'{len(files)} files with prefix {file_prefix} now combined into {cdf_filename} '+\
          f'in {ti.perf_counter()-ftic:.6f}s.')
    return True
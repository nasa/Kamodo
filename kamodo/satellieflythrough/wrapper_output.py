...
def SFdata_tocdf(filename, model_filename, model_name, results_dict, results_units):
    ...
    new_dim = data_out.createDimension('sat_time', results_dict['sat_time'].size)  #create dimension
    
    #copy over variables to file
    for key in results_dict.keys(): 
        new_var = data_out.createVariable(key, np.float32, ('sat_time'))  #create variables
...
def SFdata_tocsv(filename, model_filename, model_name, results_dict, results_units):
    ...
    for i in range(len(results_dict['sat_time'])):

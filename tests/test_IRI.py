...
def test04_times():
    ...
    '''
    dt1 = datetime.datetime(2012, 3, 30, 0, 0, tzinfo=datetime.timezone.utc)
    dt2 = datetime.datetime(2012, 4, 1, 0, 0, tzinfo=datetime.timezone.utc)
...
def test07_plot_value():
    ...
    fig = ko.plot('T_n_ijk', plot_partial={'T_n_ijk': {'time': 40., 'height': 200.}})
    assert fig.data[0]['x'][2] == pytest.approx(-170.0, abs=.000001) and \
           fig.data[0]['y'][3] == pytest.approx(-75.0, abs=.000001) and \
           fig.data[0]['z'][4,5] == pytest.approx(821.0, abs=.000001)

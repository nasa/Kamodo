...
def test05_interpolation():
    ...
    if isnan(ko.Phi([5.2, 10., 60.])[0]):
        raise AttributeError('Returned value is a NaN.')
    if isnan(ko.Phi_ijk(time=5.2, lon=10., lat=60.)):
        raise AttributeError('Returned value is a NaN.')
...
def test07_plot_value():
    ...
    fig = ko.plot('Sigma_P_ijk', plot_partial={'Sigma_P_ijk': {'lat': 60.0}})
    assert fig.data[0]['x'][2] == pytest.approx(0.06666667, abs=.000001) and \
           fig.data[0]['y'][3] == pytest.approx(-135.0, abs=.000001) and \
           fig.data[0]['z'][4,5] == pytest.approx(2.0, abs=.000001)

...
def test_times():
    ...
    '''
    dt1 = datetime.datetime(2025, 9, 17, 0, 0, tzinfo=datetime.timezone.utc)
    dt2 = datetime.datetime(2025, 9, 18, 23, 45, tzinfo=datetime.timezone.utc)
    ft = MW.File_Times(model, file_dir)
...
def test_coord_range():
    ...
    assert cr['T_n']['time'][1] == pytest.approx(47.75, abs=.000001)
...
def test_plot_value():
    ...
    fig = ko.plot('T_n_ijk', plot_partial={'T_n_ijk': {'time': 40., 'height': 250.}})
    assert fig.data[0]['x'][2] == pytest.approx(-140.0, abs=.000001) and \
           fig.data[0]['y'][3] == pytest.approx(-75.0, abs=.000001) and \
           fig.data[0]['z'][4,5] == pytest.approx(982.9483032226562, abs=.000001)

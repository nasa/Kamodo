...
model_varnames = {'Eflux_e+i_all': ['Phi_eplusiEall',
                                    'energy flux of electrons and ions in all aurora types',
                                    0, 'SM', 'sph',
                                    ['time', 'lon', 'lat'],'mW/m**2'],
                  'Eflux_e_all': ['Phi_eEall',
                                  'energy flux of electrons in all aurora type',
                                  0, 'SM', 'sph',
                                  ['time', 'lon', 'lat'],'mW/cm**2'],
                  'Eflux_i_all': ['Phi_iEall',
                                  'energy flux of ions in all aurora types',
                                  0, 'SM','sph',
                                  ['time', 'lon', 'lat'], 'muA/cm**2'],
                  'Eflux_e_mono': ['Phi_eEmono',
                                   'energy flux of electrons in mono-energetic (inverted-V) aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'mW/cm**2'],
                  'Eflux_e_diff': ['Phi_eEdiff',
                                   'energy flux of electrons in diffuse aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'mW/cm**2'],
                  'Eflux_e_wave': ['Phi_eEwave',
                                   'energy flux of electrons in broadband (wave) aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'mW/cm**2'],
                  'Eave_i_all': ['Eave_i_all',
                                   'average energy of ions in all aurora types',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'keV'],
                  'Eave_e_mono': ['Eave_e_mono',
                                   'average energy of electrons in monoenergetic (inverted-V) aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'keV'],
                  'Eave_e_diff': ['Eave_e_diff',
                                   'average energy of electrons in diffuse aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'keV'],
                  'Eave_e_wave': ['Eave_e_wave',
                                   'average energy of electrons in broadband (wve) aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], 'keV'],
                  'Nflux_i_all': ['Phi_iNall',
                                   'particle number flux of ions in all aurora types',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], '1/cm**2/s'],
                  'Nflux_e_mono': ['Phi_eNmono',
                                   'particle number flux of electrons in monoenergetic (inverted-V) aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], '1/cm**2/s'],
                  'Nflux_e_diff': ['Phi_eNdiff',
                                   'particle number flux of electrons in diffuse aurora',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], '1/cm**2/s'],
                  'Nflux_e_wave': ['Phi_eNwave',
                                   'particle number flux of electrons in broadband (wave) aurora types',
                                   0, 'SM', 'sph',
                                   ['time', 'lon', 'lat'], '1/cm**2/s']               
                  }
...
def MODEL():
    ...
    class MODEL(Kamodo):
        '''Ovarion-Prime (CCMC run-on-request) model data reader.
        ...
        def __init__(self, file_dir, variables_requested=[],
            ...
            if not RU._isfile(list_file) or not RU._isfile(time_file):
                ...
                self.times[p] = {'start_index': [], 'start': [], 'end': [], 'all': []}

                # loop through to get times
                for f in range(len(pattern_files)):
                    filename = pattern_files[f]
                    date = filename[len(filename)-11:-3]
                    new_filedate = datetime.strptime(
                        date+' 00:00:00', '%Y%m%d %H:%M:%S').replace(
                        tzinfo=timezone.utc)
                    cdf_data = RU.Dataset(pattern_files[f],
                                          filetype='netCDF3')
                    # hours since midnight first file
                    tmp = (array(cdf_data.variables['time']))/60. # time of day in hours
                    net = (new_filedate-self.filedate).days*24. + tmp

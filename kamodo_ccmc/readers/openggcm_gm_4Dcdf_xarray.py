...
def MODEL():
    ...
    class MODEL(Kamodo):
        ...
        def __init__(self, full_file_prefix, variables_requested=[],
            ...
            cdf_data = xarray.open_dataset(nc_file, chunks={'time': 100,
                                                            'x': 100, 'y': 100,
                                                            'z': 100})
            ...
            self.datetimes = [datetime.utcfromtimestamp(
                hrs_to_ts(t[0], self.filedate)).isoformat(sep=' '),
                              datetime.utcfromtimestamp(
                                  hrs_to_ts(t[-1], self.filedate)).isoformat(sep=' ')]
            self.filetimes = [hrs_to_ts(t[0], self.filedate),
                              hrs_to_ts(t[-1], self.filedate)]   # timestamps
        ...
        def register_variable(self, units, variable, varname, gridded_int):
            # variable may have different grid positions in the staggered grid of the model
            x_, y_, z_ = self.get_grid(varname)
            xvec_dependencies = {'time': 'hr', 'x': 'R_E', 'y': 'R_E', 'z': 'R_E'}

            # variable is a DataArray object with a built-in interpolator
            # need to add coordinates
            variable = variable.assign_coords({'time': self._time, 'x': x_,
                                               'y': y_, 'z': z_})
        ...
        def custom_interp(self, units, variable, varname, xvec_dependencies,
                          gridded_int):
            ...
            def interpolator(xvec):  # xvec = [[t1,x1,y1,z1],[t2,x2,y2,z2],...]
                ...
                t0 = perf_counter()
                xvec_arr = array(xvec)
                if len(xvec_arr.shape) == 1:
                    xvec_arr = expand_dims(xvec_arr, 0)
                r = sqrt(sum(xvec_arr[:, 1:]**2, axis=1))  # calculate radius
                result = zeros(len(r))  # option 2, about same time as option 1
                for i in range(len(r)):
                    if r[i] > self.near_Earth_boundary_radius:
                        result[i] = float(variable.interp(time=xvec_arr[i][0],
                                                          x=xvec_arr[i][1],
                                                          y=xvec_arr[i][2],
                                                          z=xvec_arr[i][3]).values)
                    else:
                        result[i] = NaN
            ...
            if gridded_int:
                ...
                def gridded_interpolator(time, x, y, z):
                    ...
                    return where(r > self.near_Earth_boundary_radius,
                                 variable.interp(time=time, x=x, y=y, z=z).values,
                                 NaN)

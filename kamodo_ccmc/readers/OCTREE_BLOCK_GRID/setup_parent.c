...
IDL_LONG setup_parent(IDL_LONG iblock, IDL_LONG ilev,
  ...
  if (DEBUG == 1){
    fprintf(stderr,
	    "Setup_parent: XMIN: %f YMIN: %f ZMIN: %f N_blks: %i N_parent_max: %i N_parents: %p numblocks_at_AMRlevel: %p\n",
  ...
  if (DEBUG == 1){
    fprintf(stderr,
	    "Setup_parent: Root block: %i XMIN: %f XMAX: %f YMIN: %f YMAX: %f ZMIN: %f ZMAX: %f\n",
	    root, octree_blocklist[root].XMIN, octree_blocklist[root].XMAX,
	    octree_blocklist[root].YMIN, octree_blocklist[root].YMAX,
	    octree_blocklist[root].ZMIN, octree_blocklist[root].ZMAX);
  }
  jlev=0;
  if (DEBUG >= 2){

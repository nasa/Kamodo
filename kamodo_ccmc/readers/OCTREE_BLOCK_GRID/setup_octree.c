...
void   setup_octree_pointers(
  ...
  MAX_AMRLEVELS = MAX_AMRLEVELS_in;
}
...
//  float p1,p2,p3,aspect_xy,aspect_yz,aspect_xz,
  float aspect_xy, aspect_yz, aspect_xz,
    ...
    float *xr_blk, *yr_blk, *zr_blk, *box_range;
...
/* aspect ratio of first block (all blocks have the same) */
    ...
    fprintf(stderr,"Box size: %f %f  %f %f  %f %f\n",
	    XMIN, XMAX, YMIN, YMAX, ZMIN, ZMAX);
    fprintf(stderr,"Initial root block outlay: %f %f %f N_blks:  %i AMR levels: %i\n",
...
/* sort blocks into AMR refinement levels */
    for (iblk=0;iblk<N_blks;iblk++){
	  ...
	  fprintf(stderr,"AMR level %i too high (>%i)\n", ilev, MAX_AMRLEVELS-1);
#ifndef no_idl
            success[0] = -1;
            return;
#else
    ...
    for (ilev=0; ilev<=MAX_AMRLEVELS-1; ilev++){
        fprintf(stderr, "AMR level: %i number of blocks: %i\n",
		ilev, numblocks_at_AMRlevel[ilev]);
    }
#endif    
	...
	for (iz=0; iz<p3; iz++){
	  ...
	  for (iy=0; iy<p2; iy++){
		...
		if  ( (XC > octree_blocklist[test_block].XMIN
		       && XC < octree_blocklist[test_block].XMAX)
		      && (YC > octree_blocklist[test_block].YMIN
			  && YC < octree_blocklist[test_block].YMAX)
		      && (ZC > octree_blocklist[test_block].ZMIN
			  && ZC < octree_blocklist[test_block].ZMAX)
		      )
		  {
		    block_exists = 1;
		  }
	...
	for (iz=0; iz<N_iz; iz++){
	  ...
	  for (iy=0; iy<N_iy; iy++){
		...
		if (octree_blocklist[ip].refinement_level != (ilev-1)){
		  ...
		  fprintf(stderr,
			  "Setup Octree: parent block %i in wrong refinement level %i!=%i (ilev).\n",
		...
		if (octree_blocklist[ip].child_IDs[iv_c] == -1){
		  ...
		  fprintf(stderr,
			  "Block ip=%ld:  Child block already assigned: iv_c=%ld ix_c=%ld iy_c=%ld iz_c=%ld\n",
	...
	for (iblk=0;iblk<numblocks_at_AMRlevel[ilev];iblk++){
	  ...
	  if (parent_block < 0){
	    fprintf(stderr,"setup_octree: Parent block missing for block %i\n", ib);
	    // error encountered
#ifndef no_idl
	    return;
#else
...
#ifdef DEBUG
      fprintf(stderr,
	      "Setup_Octree::new_algorithm: level: %i of %i level_factor: %f N_Parents: Total: %i Max: %i\n",

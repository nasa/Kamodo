...
extern octree_block *octree_blocklist;

extern float *fields,*x_blk,*y_blk,*z_blk,*xr_blk,*yr_blk,*zr_blk;
extern float MISSING;
extern IDL_LONG  NVV,NX,NX1,NX2,NY,NY1,NY2,NZ,NZ1,NZ2,nvar,NV_blk,NVV_blk,N_blks;

extern IDL_LONG  MAX_AMRLEVELS,parent_count,
    *block_at_AMRlevel,*numblocks_at_AMRlevel,*numparents_at_AMRlevel;


extern IDL_LONG setup_parent(IDL_LONG iblock, IDL_LONG ilev,
		      float XMIN, float YMIN, float ZMIN,
		      IDL_LONG N_blks,
		      IDL_LONG N_parents_max, 
		      IDL_LONG *N_parents);

extern IDL_LONG climb_octree(IDL_LONG root, float x, float y, float z,
		      IDL_LONG max_level);
extern IDL_LONG find_octree_block(float x, float y, float z,
			   IDL_LONG old_blocknumber, IDL_LONG max_level);
/* TOLERANCE allows for small numerical deviation from integer */
#define TOLERANCE 0.001

extern float p1,p2,p3, /* number of root blocks in box
...
extern int grid_select(float *, int, int,
                float, float, float,
                float*, long, long);

/* interpolation in AMR block */
extern float interpolate_in_block(float, float, float,
                           float*,
                           float*, float*, float*,
                           IDL_LONG, IDL_LONG, IDL_LONG);
#ifndef no_idl
extern float interpolate_amrdata(float xx, float yy, float zz,
                          char *variable_name,
...
/* function prototypes */
/***********************/
extern void sami_grid_pos(double y00, double y01, double y10,
		   double z00, double z01, double z10,
		   double y_p, double z_p,
		   double *dy, double *dz);
extern float interpolate_sami_data(const float lon_, const float lat_, const float hh_,
			    char *variable_name,
...
extern float interpolate_lfm_data(float XX, float YY, float ZZ,
			   char *variable_name,
			   IDL_LONG VAR_ID, /* offset in fields array */
			   IDL_LONG NVAR, /* stride in fields array */
			   float *field, 
...
extern IDL_LONG trace_fieldline_analytic_b(float x_start, float y_start, float z_start,
				    float r_end,
...
extern IDL_LONG trace_fieldline_lfm(
		    float *flx, float *fly, float *flz, float *v_mag,
                    float *fields,
...
extern IDL_LONG find_block(float, float, float,
                    float *, float *, float *, long,long);

extern void find_in_block(float, float, float, int*, int*, int*,
                   float* , float*, float*,
                   IDL_LONG, IDL_LONG, IDL_LONG);

extern void hunt(float *, IDL_LONG, float, int *);


extern IDL_LONG trace_fieldline(float x_start, float y_start, float z_start,
                         float r_end,
...
extern IDL_LONG trace_fieldline_spherical_staggered(
...
extern IDL_LONG trace_fieldline_staggered(float x_start, float y_start, float z_start,
                    float r_end,
                    IDL_LONG NX, IDL_LONG NY, IDL_LONG NZ, IDL_LONG N_blks,
                    float *x, float *y, float *z,
                    float *x_bx, float *y_by, float *z_bz,
...
extern IDL_LONG trace_fieldline_cdf(char *cdf_file, char *cdf_variables,
			     float *flx, float *fly, float *flz, float *v_mag,
			     float r_end,
			     IDL_LONG *step_max,
			     float dn, float bdp, float *tilt, float missing);

extern IDL_LONG trace_fieldline_octree(
    float x_start, float y_start, float z_start,
    float r_end,
...
extern IDL_LONG trace_fieldline_octree_spherical(
    float x_start, float y_start, float z_start,
    float r_end,
    float *bxfield, float *byfield, float *bzfield,
    float *flx, float *fly, float *flz, IDL_LONG *step_max,
...
extern IDL_LONG trace_particle_octree(
    float x_start, float y_start, float z_start,
    float r_end,
